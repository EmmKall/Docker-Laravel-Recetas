...
@section('content')
    <h2 class="text-center h1">Editar Receta</h2>

    <div class="col-md-11 my-3 mx-auto border rounded rounded-3">
        <form action="{{ route('recetas.update', ['receta' => $receta->id]) }}" method="POST" class="row bg-primary border border-3 border-primary" novalidate enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="col-12 col-md-3 form-group bg-primary p-1 rounded-3 text-center">
                <label for="titulo" class="col-form-label text-white">{{ __('Titulo Receta') }}</label>
                <input type="text" id="titulo" name="titulo" placeholder="Ingrese un titulo" class="form-control @error('titulo') is-invalid @enderror" value="{{ $receta->titulo }}" />
            ...
            <div class="col-12 col-md-3 text-center text-white">
                <label for="">{{ __('Imagen actual') }}</label>
                 @if($receta->imagen)
                    <div class="imagen d-flex justify-content-center align-items-center">
                        <img style="width: 180px;" src="../../storage/{{ $receta->imagen }}" class="card-img-top" alt="Receta">
                    </div>
                 @endif
            </div>
            ...
            <div class="col-12 col-md-6 form-group bg-primary p-1 rounded-3">
                ...
                <input type="hidden" id="ingredientes" name="ingredientes" placeholder="Ingredientes" class="form-control" value="{{ $receta->ingredientes }}" />
            ...
            <div class="col-12 col-md-6 form-group bg-primary p-1 rounded-3">
                ...
                <input type="hidden" id="preparacion" name="preparacion" placeholder="preparacion" class="form-control @error('preparacion') is-invalid @enderror" value="{{ $receta->preparacion }}" />
